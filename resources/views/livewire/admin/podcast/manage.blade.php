                                ...
                                <div class="inline-flex items-center rounded-lg border border-gray-200 bg-gray-50">
                                    ...
                                    @if($canReview)
                                        ...
                                        <div class="flex flex-wrap items-center gap-1.5 px-2 py-1" aria-label="Review actions">
                                            @if($episode->approval_status !== 'approved')
                                                <button wire:click="startApproval({{ $episode->id }}, 'approved')" wire:loading.attr="disabled" class="inline-flex items-center gap-1 rounded-md bg-emerald-100 px-2 py-1.5 text-xs font-semibold text-emerald-700 transition hover:bg-emerald-200 disabled:opacity-50" title="Approve episode"><i class="fas fa-check-circle" aria-hidden="true"></i><span>Approve</span></button>
                                            @endif
                                            @if($episode->approval_status !== 'flagged')
                                                <button wire:click="startApproval({{ $episode->id }}, 'flagged')" wire:loading.attr="disabled" class="inline-flex items-center gap-1 rounded-md bg-amber-100 px-2 py-1.5 text-xs font-semibold text-amber-700 transition hover:bg-amber-200 disabled:opacity-50" title="Flag episode"><i class="fas fa-flag" aria-hidden="true"></i><span>Flag</span></button>
                                            @endif
                                            @if($episode->approval_status !== 'rejected')
                                                <button wire:click="startApproval({{ $episode->id }}, 'rejected')" wire:loading.attr="disabled" class="inline-flex items-center gap-1 rounded-md bg-red-100 px-2 py-1.5 text-xs font-semibold text-red-700 transition hover:bg-red-200 disabled:opacity-50" title="Reject episode"><i class="fas fa-times-circle" aria-hidden="true"></i><span>Reject</span></button>
                                            @endif
                                        </div>
                                    @endif
