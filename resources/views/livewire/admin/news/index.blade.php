                        ...
                        @if($canReview)
                            <div class="mt-3 flex flex-wrap justify-end gap-2 border-t border-slate-100 pt-3" aria-label="Review actions">
                                @if($article->approval_status !== 'approved')
                                    <button wire:click="startApproval({{ $article->id }}, 'approved')" wire:loading.attr="disabled" class="inline-flex items-center gap-1.5 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-700 shadow-sm transition hover:border-emerald-300 hover:bg-emerald-100 disabled:opacity-50"><i class="fas fa-check-circle" aria-hidden="true"></i>Approve</button>
                                @endif
                                @if($article->approval_status !== 'flagged')
                                    <button wire:click="startApproval({{ $article->id }}, 'flagged')" wire:loading.attr="disabled" class="inline-flex items-center gap-1.5 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-bold text-amber-700 shadow-sm transition hover:border-amber-300 hover:bg-amber-100 disabled:opacity-50"><i class="fas fa-flag" aria-hidden="true"></i>Flag</button>
                                @endif
                                @if($article->approval_status !== 'rejected')
                                    <button wire:click="startApproval({{ $article->id }}, 'rejected')" wire:loading.attr="disabled" class="inline-flex items-center gap-1.5 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs font-bold text-red-700 shadow-sm transition hover:border-red-300 hover:bg-red-100 disabled:opacity-50"><i class="fas fa-times-circle" aria-hidden="true"></i>Reject</button>
                                @endif
                            </div>
                        @endif
                    </div>
