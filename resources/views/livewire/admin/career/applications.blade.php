                    ...
                    <main class="space-y-5 p-4 sm:p-6">
                        <section class="rounded-2xl border border-emerald-200 bg-white p-5 shadow-sm">
                            <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                                <div><h3 class="text-sm font-black uppercase tracking-wide text-slate-900"><i class="fas fa-list-check mr-2 text-emerald-600"></i>Application status</h3><p class="mt-1 text-xs leading-5 text-slate-500">Choose where this applicant currently sits in your review workflow.</p></div>
                                <span class="w-fit rounded-full px-3 py-1 text-[10px] font-extrabold uppercase {{ $statusStyles[$selectedApplication->status] ?? 'bg-slate-100 text-slate-600' }}">Current: {{ \Illuminate\Support\Str::headline($selectedApplication->status) }}</span>
                            </div>
                            <div class="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                                @foreach([
                                    'new' => ['New', 'Not reviewed yet', 'fa-sparkles'],
                                    'reviewing' => ['Reviewing', 'Currently being assessed', 'fa-magnifying-glass'],
                                    'shortlisted' => ['Shortlisted', 'Move to the next stage', 'fa-star'],
                                    'hired' => ['Hired', 'Applicant was selected', 'fa-user-check'],
                                    'rejected' => ['Rejected', 'Applicant was not selected', 'fa-user-xmark'],
                                    'archived' => ['Archived', 'Keep outside the active queue', 'fa-box-archive'],
                                ] as $status => [$label, $description, $icon])
                                    <button type="button" wire:click="setStatus({{ $selectedApplication->id }}, '{{ $status }}')" wire:loading.attr="disabled" wire:target="setStatus" class="flex items-center gap-3 rounded-xl border p-3 text-left transition disabled:opacity-60 {{ $selectedApplication->status === $status ? 'border-emerald-500 bg-emerald-50 ring-2 ring-emerald-100' : 'border-slate-200 hover:border-emerald-300 hover:bg-slate-50' }}" aria-pressed="{{ $selectedApplication->status === $status ? 'true' : 'false' }}">
                                        <span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg {{ $statusStyles[$status] ?? 'bg-slate-100 text-slate-600' }}"><i class="fas {{ $icon }}"></i></span>
                                        <span class="min-w-0"><span class="block text-xs font-black text-slate-900">{{ $label }}</span><span class="mt-0.5 block text-[10px] leading-4 text-slate-500">{{ $description }}</span></span>
                                        @if($selectedApplication->status === $status)<i class="fas fa-circle-check ml-auto text-emerald-600" aria-hidden="true"></i>@endif
                                    </button>
                                @endforeach
                            </div>
                        </section>
