    ...
    <main>
        ...
        @elseif($section === 'gallery')
            <section class="mx-auto max-w-7xl px-5 py-16 sm:px-8 lg:px-10 lg:py-24">
                @if($categories->isNotEmpty())<div class="mb-8 flex flex-wrap gap-2">@foreach($categories as $category)<a href="{{ route('vettas.categories.show', $category) }}" class="border border-[#0b1830]/15 bg-white px-4 py-2 text-sm font-semibold hover:border-[#f36b21]">{{ $category->name }}</a>@endforeach</div>@endif
